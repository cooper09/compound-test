...
//cooper s - convert to BigInt (whatever that means...)
...
console.log("BuySwap - Swap Eth for Dai trade: ");
        ...
        //check for production/dev nonce ...add 1 to production
        if(!orderId) {
            console.log("swap/buy")
            // cooper s - turn on for production (swap/buy only)
            currentNonce = currentNonce +1;
        }

        if(orderId) {
            console.log("Buy/swap")
            //currentNonce = currentNonce +1;  //turn off for production only
        }
        ...
        const rawTxn = await router.populateTransaction.swapExactETHForTokens(
            ...
            if (receipt) {
                ...
                await logger("logger "+timestamp+" - BuySwap - Buy transaction complete: "+ (await sendTxn).hash );
            }

} catch(e) {
    console.log("Something went wrong with buy transaction: ", e.message )
    process.exit(0)
}//end try/catch
